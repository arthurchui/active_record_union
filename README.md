...
1. ActiveRecordUnion with raise an error if you try to UNION any relations that do any eager-loading. There's no sensible way to do the including in the subselects. If enough people complain maybe we can change ActiveRecordUnion to let the queries run anyway but without including any records.
